...
async function seed() {
  ...
  const { data: topics, error: topicsErr } = await supabaseAdmin.from('topics').select('id, name')
  ...
  console.log(`Prepared ${rows.length} subtopics`)

  const { data: existing, error: existErr } = await supabaseAdmin.from('subtopics').select('topic_id, name')
  if (existErr) {
    console.error('Error fetching existing subtopics:', existErr)
    return
  }
  ...
  if (newRows.length === 0) {
    console.log('All done, no new subtopics to insert!')
    return
  }
  ...
  for (let i = 0; i < newRows.length; i += BATCH) {
    ...
    const { error } = await supabaseAdmin.from('subtopics').insert(batch)
    if (error) {
